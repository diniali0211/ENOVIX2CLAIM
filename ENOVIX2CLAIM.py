...
- Claim cycle is **29 → 28** (applies automatically to every month).
"""
)
...
if att_file and mst_file:
    try:
        ...
        # Claim can only start after 30 days from join
    ...
    (att["__Date"] <= att["ELIGIBLE_END"])
        ...
        for cyc in sorted(daily["Cycle"].unique()):
            ...
            base = (
    sub.groupby(
        ["__EmpID","__Name","JOIN_DATE","CLAIM_START","ELIGIBLE_END","Recruiter"],
        as_index=False
    )
    .agg(
        Total_Hours=("Total_Hours","sum"),
        Claim_Days=("Worked_Day","sum")
    )
)
          ...
          + [str(d) for d in day_cols] \
          + ["TOTAL WORKING","Total_Hours"]
        ...
        if combined_claims:
            ...
            st.subheader("🧾 Combined Claim Table (All 29→28 Cycles)")
        ...
        if all_summary:
            ...
            st.markdown("### 👥 Combined Per-Recruiter Summary")
        ...
        st.download_button(
            "⬇️ Download Excel (one sheet per 29→28 cycle: table + summary + grand total)",
